...
class Meteor(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.rect = self.image.get_rect(center=(random.randint(0, WIDTH), random.randint(-200, -40)))
        self.speed = random.randint(1, 3)
        self.health = size // 10  # чем больше, тем крепче
        ...
        while running:
            ...
            if not game_over:
                ...
                if self.spawn_timer > 30:
                    ...
                    if random.randint(1, 100) < 30:  # шанс 5% каждый кадр
                        meteor = Meteor()
                ...
                for bullet in self.bullets:
                    hit_meteor = pygame.sprite.spritecollideany(bullet, self.meteors)
                    if hit_meteor:
                        bullet.kill()
                        hit_meteor.health -= 1
                        if hit_meteor.health <= 0:
                            hit_meteor.kill()
                ...
                for meteor in self.meteors:
                    if self.player.rect.inflate(-40, -20).colliderect(meteor.rect.inflate(-40, -20)):
                        game_over = True
                        break
